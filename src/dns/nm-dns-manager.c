...
rebuild_domain_lists (NMDnsManager *self)
{
	...
	c_list_for_each_entry (ip_data, head, ip_config_lst) {
		...
		if (nm_ip_config_best_default_route_get (ip_config))
			 ...
			 * route (this can happen for example when the server pushes
			 * routes with openconnect), and there are no search domains,
			 * then the name servers pushed by the server would be unused.
			 * It is preferable in this case to use the VPN DNS server for
			 * all queries. */
